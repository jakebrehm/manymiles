...
class User(Base):
    ...
    password_id = Column("password_id", ForeignKey("password.id", name="fk_user_password"), nullable=True)
...
class Record(Base):
    ...
    user_id = Column(Integer, ForeignKey("user.id", name="fk_record_user"))
...
class Login(Base):
    ...
    user_id = Column("user_id", ForeignKey("user.id", name="fk_login_user"))
...
class Password(Base):
    ...
    user_id = Column("user_id", ForeignKey("user.id", name="fk_password_user"), nullable=False)
    password_hash = Column("password_hash", String(64), nullable=False)
